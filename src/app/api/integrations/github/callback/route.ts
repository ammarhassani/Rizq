...
function back(locale: string, status: string): string {
...
export async function GET(req: NextRequest) {
  ...
  if (!code || !state || !savedState || state !== savedState) {
    return NextResponse.redirect(back(locale, "error"));
  }
  store.delete("gh_oauth_state");

  if (!isGithubConfigured() || !isTokenCryptoConfigured()) {
    return NextResponse.redirect(back(locale, "not_configured"));
  }

  const supabase = await createClient();
  const { data: userResult } = await supabase.auth.getUser();
  if (!userResult.user) return NextResponse.redirect(back(locale, "error"));

  const exchanged = await exchangeGithubCode(code);
  if (!exchanged) return NextResponse.redirect(back(locale, "error"));
  ...
  return NextResponse.redirect(back(locale, "connected"));
}
