...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  const tI18n = await getTranslations({ locale: locale as "ar" | "en", namespace: "Settings" });
  const isAr = locale === "ar";
  return { title: `${tI18n("profile")} · رِزق` };
}
...
export default async function SettingsProfilePage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <AppShell locale={locale as "ar" | "en"} title={tI18n("profile")} maxWidth="reading">
      <div dir={dir} className={font}>
        ...
        <div className="mb-6">
          <p className="eyebrow mb-2">{tI18n("profile")}</p>
          <h1 className={`display-2 text-rizq-ink ${font}`}>
            {tI18n("completeProfile2")}
          </h1>
