...
/**
 * ShareInvoiceModal — Phase-4 task P4.5.
 *
 * Modal/sheet for sharing an invoice.
 * - Toggle sharing on/off via setInvoiceShare.
 * - Once enabled: copy link, open PDF (share-page print), WhatsApp deep link.
 * - Sharing a draft invoice will bump it to 'sent' (shown in UI note).
 * - Logs whatsapp/pdf_download via logInvoiceShareChannel (best-effort).
 * Mirrors ShareModal (proposals) 1:1 with invoice-specific keys.
 */

import { useState, useTransition, useRef, useEffect } from "react";
import { X, Check, Copy, ExternalLink, Loader2 } from "lucide-react";
import { useTranslations } from "next-intl";
import { setInvoiceShare, logInvoiceShareChannel } from "@/app/actions/invoices/shareActions";
...
type Props = {
  ...
  /** Current invoice status — used to show a note when draft will become 'sent' */
  status: string;
  onClose: () => void;
};
...
export function ShareInvoiceModal({
  ...
  const t = useTranslations("Invoices.share");
  ...
  const shareUrl =
    token && typeof window !== "undefined"
      ? `${window.location.origin}/${locale}/i/${token}`
      : null;
  ...
  function handleToggle() {
    ...
    startTransition(async () => {
      const result = await setInvoiceShare({
  ...
  function handlePdf() {
    if (!shareUrl) return;
    logInvoiceShareChannel({ token: token!, channel: "pdf_download" });
    window.open(shareUrl, "_blank", "noopener,noreferrer");
  }

  function handleWhatsApp() {
    if (!waHref || !token) return;
    logInvoiceShareChannel({ token, channel: "whatsapp" });
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Draft notice — sharing will mark invoice as sent */}
        {!isShared && status === "draft" && (
          <p className={`text-xs text-rizq-gold-dark bg-rizq-gold/10 rounded-xl px-4 py-3 mb-4 ${font}`}>
            {t("draftWillBeSent")}
          </p>
        )}
