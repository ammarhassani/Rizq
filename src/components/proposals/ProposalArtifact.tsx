...
type Props = {
  ...
  proposalId?: string;
  /**
   * Who is reading this document. "owner" (default) is the freelancer's working
   * view and carries the per-section AI review badges. "client" is the copy the
   * recipient sees — a badge reading "review before sending" is an instruction to
   * the freelancer and has no business on the client's copy; the document-level
   * "made with Rizq" line already discloses the tool.
   */
...
/**
 * "Valid until" wants a date, not a duration — the cover previously read
 * "صالح حتى: 30 يومًا" ("Valid until: 30 days"), which is a label/value mismatch a
 * client has to do arithmetic on. Falls back to the duration when we have no
 * issue date to count from.
 */
function fmtValidUntil(
  ...
  if (issueDate) {
    ...
    if (!Number.isNaN(d.getTime())) {
      ...
      return fmtDate(d.toISOString(), locale);
    }
  }
  return t.validityDays(days);
}
...
function SectionShell({
  ...
  return (
    <section
      // Theme token, not a hardcoded white: every text colour in here is
      // token-driven, so a fixed light card left dark-mode readers with pale text
      // on a pale card. Print keeps its own white via ProposalPrintStyles.
      className={`rounded-2xl border border-rizq-gold/20 bg-[var(--raised)] p-6 sm:p-8 break-inside-avoid ${className}`}
...
// --- Honesty badge (shown on AI-drafted prose; owner view only — the client copy
// is stripped of the ai_generated flags upstream by forClientAudience) ---
      ...
      <div dir={dir}>
        ...
        >
          ...
          <span>
            {t.validUntil}:{" "}
            <span className="text-rizq-ink font-medium">
              {fmtValidUntil(issueDate, validityDays, locale, t)}
            </span>
          </span>
        </div>
      </div>
...
function VerificationSection({
  ...
  const t = T[locale];
  const c = section.content;
  // The client reads this document — the footer follows the document's language,
  ...
  return (
    <SectionShell title={t.sections.terms_footer}>
      <div className={`flex flex-wrap gap-x-6 gap-y-2 text-sm text-rizq-ink-soft ${font}`}>
        ...
        <span>
          {t.validUntil}:{" "}
          <span className="text-rizq-ink">
            {fmtValidUntil(str(c["issueDate"] as string | null | undefined), validityDays, locale, t)}
          </span>
        </span>
      </div>
    </SectionShell>
...
export function ProposalArtifact({
  ...
  // Sort by order in case storage has them out of order. The client's copy drops
  // the per-section "AI-drafted, review before sending" badges — that is an
  // instruction to the freelancer, not something the recipient should read.
  const source = audience === "client" ? forClientAudience(data) : data;
  const sections = [...source.sections].sort((a, b) => a.order - b.order);
