...
const FIELDS: Array<Omit<StrengthItem, "met"> & { met: (p: StrengthProfile) => boolean }> = [
  ...
  { key: "specialty", label_ar: "التخصص", label_en: "Specialty", met: (p) => Boolean(p.primary_specialty_id) },
  { key: "city", label_ar: "المدينة", label_en: "City", met: (p) => Boolean(p.city_id) },
  { key: "experience", label_ar: "مستوى الخبرة", label_en: "Experience level", met: (p) => Boolean(p.experience_tier_id) },
